...
export const AQLSamplesPage: React.FC = () => {
  ...
  const handleScanSample = async (code: string) => {
    ...
    return {
      ...
      message: `Sample ${currentIdx}/${session.sampleRequired} scanned: ${code}`,
      code,
    };
  };
  ...
  const handleNextSample = async () => {
    ...
    if (currentIdx < session.sampleRequired) {
      ...
      showToast(`Recorded Sample ${currentIdx}/${session.sampleRequired}. Move to Sample ${nextIndex}`, 'info');
    } else {
      ...
      showToast(`All ${session.sampleRequired} samples saved to database. AQL inspection complete!`, 'success');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={styles.stepBar}>
        ...
        <div style={styles.stepActive}>
          ...
          <span>Samples ({currentIdx}/3)</span>
        </div>
        <div style={styles.stepDivider} />
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h3 style={{ fontSize: '16px', fontWeight: 700 }}>
          Sample Item {currentIdx} of {session.sampleRequired}
        </h3>
        <StatusPill label={`Sample ${currentIdx}/3`} variant="purple" />
      </div>
